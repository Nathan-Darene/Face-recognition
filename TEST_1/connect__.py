...
from PIL import Image, ImageTk  # Pour afficher l'image sélectionnée
from TEST_1.zoom_image import zoom_image
...
def display_image(image_path):
    ...
    # Appliquer un zoom léger sur l'image
    zoom_factor = 1.20  # Par exemple, zoomer de 20%
    zoomed_image = zoom_image(image, zoom_factor)
    # Convertir l'image zoomée en un format compatible avec CustomTkinter
    image_tk = ctk.CTkImage(light_image=zoomed_image)    # image_tk = ctk.CTkImage(light_image=image, size=(200, 200))
...
button_delete_face = ctk.CTkButton(app, text="Supprimer un visage", command=go_back_to_main_menu)

...
while True:
    ...
    for face_encoding, (top, right, bottom, left) in zip(face_encodings, face_locations):
        ...
        for (x, y, w, h) in faces:
            ...
            for i in range(0):  # Il y a 68 points dans le modèle
                part = shape.part(i)
                cv2.circle(frame, (part.x, part.y), 2, (0, 255, 0), -1)  # Dessiner un petit cercle à chaque landmark

            # Afficher un texte fixe lorsqu'un visage est détecté
            text = f"{name}"
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
            text_x = x
            text_y = y - 10  # Position du texte juste au-dessus du visage
        ...
        print(f"Texte à afficher : {text} à ({text_x}, {text_y})")
